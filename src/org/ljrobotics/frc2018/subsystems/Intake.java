...
public class Intake extends Subsystem implements LoopingSubsystem {
	...
	public static Intake getInstance() {
		if (instance == null) {
			...
			instance = new Intake(left, right, tension, leftDistance, rightDistance);
		}
		return instance;
	}
	...
	private long isAlignedTimerStart;
	...
	public boolean isAligned() {
		...
		boolean distanceThreshold = Math.abs(distanceDifference) < 0.1;
		boolean isAligned = false;
		if (distanceThreshold && this.isAlignedTimerStart == 0) {
			this.isAlignedTimerStart = System.currentTimeMillis();
		} else if (distanceThreshold && !(this.isAlignedTimerStart == 0)
				&& System.currentTimeMillis() - this.isAlignedTimerStart > 0.1D) {
			this.isAlignedTimerStart = 0;
			isAligned = true;
		}
		return isAligned;
	...
	protected void setAlignMotors() {
		this.setSpeed(Constants.ROTATION_SPEED, Constants.ROTATION_SPEED);
	}
	...
	public void intakeDecisions() {
		...
		if (!front_beam.get() && !back_beam.get()) { // No box in, go find one
			...
			this.setSpeed(Constants.SUCK_SPEED, -Constants.SUCK_SPEED);
	...
	protected void initDefaultCommand() {
		// this.setDefaultCommand(new IntakeAlign());
	}
